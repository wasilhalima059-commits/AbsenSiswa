...
if(u.pathname==='/api/students'){const cls=u.searchParams.get('class')||'',year=u.searchParams.get('year')||'',key=year?`${cls}__${year}`:cls,db=read(STU);if(req.method==='GET')return send(res,200,db[key]||db[cls]||[]);if(req.method==='PUT'){const b=await body(req);db[key]=Array.isArray(b)?b:[];write(STU,db);return send(res,200,{ok:true})}}
